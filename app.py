...
# Etiqueta que se pinta en la pestaña: si el grupo tiene más de un item
# (es "desplegable"), se le añade una pequeña flecha (▾) al final del texto.
# No cambia color, tipografía ni estilo — es solo un carácter más dentro
# del mismo label, así que hereda automáticamente la estética existente.
for _g in GRUPOS_NAV:
    _g["label_visible"] = _g["label"] + ("  ▾" if len(_g["items"]) > 1 else "")

# GRUPOS_LABELS se usa como identificador interno (session_state, lookups);
# GRUPOS_LABELS_VISIBLES es lo que efectivamente se muestra en cada pestaña.
GRUPOS_LABELS          = [g["label"]         for g in GRUPOS_NAV]
GRUPOS_LABELS_VISIBLES = [g["label_visible"] for g in GRUPOS_NAV]
GRUPOS_ICONOS          = [g["icon"]          for g in GRUPOS_NAV]
# Mapas de ida y vuelta entre el label mostrado y el label interno, para
# poder usar cualquiera de los dos widgets (sac.tabs / option_menu / radio)
# sin tocar el resto de la lógica de la app.
_LABEL_VISIBLE_A_INTERNO = dict(zip(GRUPOS_LABELS_VISIBLES, GRUPOS_LABELS))
...
with col_nav:
    if ANTD_DISPONIBLE:
        ...
        grupo_activo_visible = sac.tabs(
            items=[sac.TabsItem(label=l, icon=i) for l, i in zip(GRUPOS_LABELS_VISIBLES, GRUPOS_ICONOS)],
        ...
        grupo_activo = _LABEL_VISIBLE_A_INTERNO[grupo_activo_visible]
        st.markdown('</div>', unsafe_allow_html=True)
    elif OPTION_MENU_DISPONIBLE:
        st.markdown('<div class="ds-topnav">', unsafe_allow_html=True)
        grupo_activo_visible = option_menu(
            menu_title=None,
            options=GRUPOS_LABELS_VISIBLES,
            icons=GRUPOS_ICONOS,
        ...
        grupo_activo_visible = st.radio(
            "Navegación", GRUPOS_LABELS_VISIBLES, index=idx_grupo_actual,
